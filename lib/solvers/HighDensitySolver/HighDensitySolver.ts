...
export class HighDensitySolver extends BaseSolver {
  ...
  private parallelNodeStep(): void {
    if (!this.nodePool) {
      this.nodePool = new HdNodePool(parallelHdNodesEnabled(), {
    ...
    while (this.unsolvedNodePortPoints.length > 0) {
      const next = this.unsolvedNodePortPoints[this.unsolvedNodePortPoints.length - 1]!
  ...
  _step() {
    ...
    if (this.activeSubSolver) {
      ...
      if (process.env.PERF_NODE_TIMING && (this.activeSubSolver.solved || this.activeSubSolver.failed)) {
        const g = globalThis as any
        g.__nodeTimings ??= []
        g.__nodeTimings.push({
          nodeId: (this as any)._nodeId,
          ms: Date.now() - ((this as any)._nodeStartedAt ?? Date.now()),
    ...
    if (process.env.PERF_NODE_TIMING) {
      ;(this as any)._nodeStartedAt = Date.now()
      ;(this as any)._nodeId = node.capacityMeshNodeId
    }
    this.updateCacheStats()
  }
