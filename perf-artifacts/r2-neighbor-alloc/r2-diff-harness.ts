...
const buildCase = (
  ...
  const addPort = (regionA: number, regionB: number, x: number, y: number) => {
    ...
    portX.push(x)
    portY.push(y)
    portZ.push(rand() < 0.5 ? 0 : 1)
    portAngle1.push(Math.floor(rand() * 360))
    portAngle2.push(Math.floor(rand() * 360))
  }
  ...
  const routeCount = 2 + Math.floor(rand() * 6) // 2..7
  ...
  for (let i = 0; i < routeCount; i++) {
    ...
    routeStartPort[i] = a
    routeEndPort[i] = b
    routeNet[i] = rand() < 0.3 ? 0 : i // some shared nets
  }
  const regionNetId = new Int32Array(regionCount).fill(-1)
  for (let r = 0; r < regionCount; r++) {
    if (rand() < 0.15) regionNetId[r] = Math.floor(rand() * routeCount)
  }
  const portSectionMask = new Int8Array(portCount).fill(1)
  for (let p = 0; p < portCount; p++) {
    if (rand() < 0.05) portSectionMask[p] = 0
  }
